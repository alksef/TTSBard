    ...
    O: fmt::Debug + fmt::Display + AsRef<dyn Op> + AsMut<dyn Op> + Clone + 'static,
{
    pub fn add_node(
        ...
        if !name.is_empty() {
            let mut name_cand = name.clone();
            let mut count = 0;
            while self.nodes.iter().any(|n| n.name == name_cand) {
                count += 1;
                name_cand = format!("{name}_{count}");
            }
            name = name_cand;
        }
    ...
    pub fn rename_node(&mut self, id: usize, name: &str) -> TractResult<()> {
        ...
        if !name.is_empty() {
            let mut name_cand = name.clone();
            let mut count = 0;
            while self.nodes.iter().any(|n| n.id != id && n.name == name_cand) {
                count += 1;
                name_cand = format!("{name}_{count}");
            }
            name = name_cand;
