...
## Решение лицензионного блокера

Выбран вариант 1: оригинальный код TTSBard переведён на `GPL-3.0-only`.
Решение и обязательства распространения закреплены в
[DECISION-017](../../decisions/017-gpl-3-license.md). Лицензии отдельных Piper
voice models по-прежнему проверяются независимо и не наследуются от приложения.
