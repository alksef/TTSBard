...
- Реализован третий typing output mode `Item` с явными идемпотентными командами
  show/hide для загруженного предмета VTube Studio.
- Добавлены разрешение актуального `instanceID`, отдельные статусы item action,
  восстановление через refresh и coalescing последнего желаемого typing-state.
- Автоматические проверки Rust, frontend, IPC, сборки и документации пройдены.
- 2026-07-30 пользователь подтвердил ручную проверку с реальным VTube Studio
  item: управление видимостью работает корректно.
