...
Шаги 1–2 реализованы. Предложенный шаг 3 позднее снят как общий backlog:
декомпозиция `AppState` не выполняется без конкретного дефекта или изменения,
которому мешает текущая граница. Trigger-based правило закреплено в
...
### Шаг 3 — декомпозиция AppState (историческое предложение, снято)
...
**Не планируется без конкретного сигнала:**
→ Шаг 3 не является самостоятельным backlog. Owner API выделяется по одному
seam только под наблюдаемый дефект или изменение согласно DECISION-004.
