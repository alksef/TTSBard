...
Статус остаётся `deferred`. Эта запись хранит контекст нескольких возможных
извлечений, а не готовое задание одному исполнителю. Возвращаться к ней при
конкретном изменении домена, которому мешают прямой доступ к блокировкам,
сложная изоляция теста или воспроизводимая проблема владения состоянием.

Перед запуском выбрать ровно один домен, перечислить его callers и создать
локальную задачу в `.work/ai/` с разрешёнными файлами и наблюдаемым критерием
готовности. Значимый выбор границы сервиса согласуется с пользователем.
Не начинать рефакторинг только ради уменьшения размера `state.rs`.

После каждого извлечения обновлять оставшийся scope. Когда перечисленные
проблемы устранены либо потеряли актуальность, перенести результат в архитектуру
или decision и удалить task вместе с записью в индексе.
...
- Проходят `./scripts/cargo.ps1 test --manifest-path src-tauri/Cargo.toml` и
