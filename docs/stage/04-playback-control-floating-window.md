# Stage: Очередь и управление воспроизведением (плавающее окно)
...
1. **Когда показывать окно:** (a) всегда пока приложение запущено; (b) только во время
   воспроизведения (auto-show/hide по `PlaybackStarted/Finished`) — **рекомендация (b)**,
   как OBS «Hide on Pause».
2. **5 последних фраз:** источник — persistent-история из плана `05` (phrase_history);
   клик по фразе → повторно озвучить (команда `replay_phrase`).
3. **Replay = с начала текущей** vs **replay последней отправленной** — уточнить
   (вероятно оба: 🔁 на текущей, + клик по фразе в списке).
4. **Stop vs Cancel:** stop = остановить и забыть; cancel = остановить + убрать из очереди.
5. **Очередь:** нужна ли видимость очереди (несколько фраз подряд через F6-режим) или
   достаточно «текущая + история».
...
## Рекомендация
**Вариант 1** (PlaybackManager + новое окно `playback-control`), auto-show/hide по событиям,
5 фраз из persistent-истории, hotkeys для pause/stop/replay. Естественно ложится на rodio Sink
(уже умеет pause/resume) и оконную инфраструктуру soundpanel.
