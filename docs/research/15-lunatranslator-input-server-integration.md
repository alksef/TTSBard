...
> Вывод этого исследования сохраняется для hook, эмуляторов, перевода и
> расширенной OCR-автоматизации. Минимальный one-shot OCR позднее выделен в
> самостоятельную функцию TTSBard: он устраняет необходимость запускать вторую
> программу ради единственного захвата текста, но не заменяет LunaTranslator.
