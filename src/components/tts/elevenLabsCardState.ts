...
export interface ElevenLabsFirstLoadDecision {
  loadModels: boolean
  loadVoices: boolean
}

/**
 * Decide which catalogs to load after a key save. A changed key always loads
 * both; an unchanged key only loads catalogs that are still empty.
 */
export function decideElevenLabsFirstLoad(options: {
  keyChanged: boolean
  modelsEmpty: boolean
  voicesEmpty: boolean
}): ElevenLabsFirstLoadDecision {
  return {
    loadModels: options.keyChanged || options.modelsEmpty,
    loadVoices: options.keyChanged || options.voicesEmpty,
  }
}

// ---------------------------------------------------------------------------
// Independent catalog refresh state (models and voices share the same shape)
// ---------------------------------------------------------------------------

export interface ElevenLabsCatalogState<T> {
  items: T[]
  loading: boolean
  error: string | null
}

export function createElevenLabsCatalogState<T>(): ElevenLabsCatalogState<T> {
  return { items: [], loading: false, error: null }
}

/**
 * Begin a refresh. While a refresh is already in flight the state is returned
 * unchanged so overlapping calls cannot interleave.
 */
export function startElevenLabsCatalogRefresh<T>(
  state: ElevenLabsCatalogState<T>,
): ElevenLabsCatalogState<T> {
  if (state.loading) return state
  return { ...state, loading: true, error: null }
}

/** Complete a refresh: replace the items and clear the error. */
export function completeElevenLabsCatalogRefresh<T>(
  state: ElevenLabsCatalogState<T>,
  items: T[],
): ElevenLabsCatalogState<T> {
...
/** Fail a refresh: keep the previous items and surface an inline error. */
export function failElevenLabsCatalogRefresh<T>(
  state: ElevenLabsCatalogState<T>,
  error: string,
): ElevenLabsCatalogState<T> {
