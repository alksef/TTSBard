...
# Sync Push Script: Upload local code to Windows machine
...
echo "=== Syncing source code to Windows ($REMOTE_USER@$REMOTE_HOST) ==="
...
# --delete to remove deleted files on Windows
echo "Running rsync..."
rsync -rltgoDzv --delete "${EXCLUDE_FLAGS[@]}" $REMOTE_RSYNC_PATH "$LOCAL_PATH" "$REMOTE_USER@$REMOTE_HOST:$REMOTE_PATH"
...
if [ $? -eq 0 ]; then
    ...
    echo "   Tip: Connect via SSH and run: ssh $REMOTE_USER@$REMOTE_HOST \"mkdir -Force $REMOTE_PATH\""
fi
